{
  config,
  self,
  lib,
  pkgs,
  ...
}: let
  inherit (config.lib.formats.rasi) mkLiteral;        # Theme.rasi alternative. Add Theme here
  colors = import ../themes/colors.nix;
in
{
  # home = {
  #   packages = with pkgs; [
  #     rofi-power-menu
  #   ];
  # };

  programs = {
    rofi = {
      enable = false;
      #terminal = "${pkgs.foot}/bin/foot";           # Foot is default terminal emulator
      location = "center";
      #packages = pkgs.rofi.override {plugins = [ pkgs.rofi-wayland]; };
      #font = "ComicCodeLigatures 18";
      theme =  with colors.scheme.doom; {
        "*" = {
          bg0 = mkLiteral "#${bg}";
          bg1 = mkLiteral "#414868";
          fg0 = mkLiteral "#${text}";
          fg1 = mkLiteral "#${text-alt}";
          bg3 = mkLiteral "#1A733E8F2";

          background-color = mkLiteral "transparent";
          text-color = mkLiteral "@fg0";

          margin = 0;
          padding = 0;
          spacing = 0;
        };

        "element-icon, element-text, scrollbar" = {
          cursor = mkLiteral "pointer";
        };

        "window" = {
          location = mkLiteral "center";
          width = mkLiteral "30%";
          x-offset = mkLiteral "8px";
          y-offset = mkLiteral "34px";

          background-color = mkLiteral "@bg0";
          border = mkLiteral "1px";
          border-color = mkLiteral "@bg1";
          border-radius = mkLiteral "24px";
        };

        "inputbar" = {
          spacing = mkLiteral "8px";
          padding = mkLiteral "4px 8px";
          children = mkLiteral "[ entry ]";
          background-color = mkLiteral "@bg0";
        };

        "entry, element-icon, element-text" = {
          vertical-align = mkLiteral "0.5";
        };

        "textbox" = {
          padding = mkLiteral "4px 8px";
          background-color = mkLiteral "@bg0";
        };

        "listview" = {
          padding = mkLiteral "4px 4px";
          lines = 8;
          columns = 1;
          scrollbar = false;
          dynamic = "true";
          background-color = "transparent";
        };

        "element" = {
          padding = mkLiteral "4px 8px";
          spacing = mkLiteral "8px";
        };

        "element normal urgent" = {
          text-color = mkLiteral "@fg1";
        };

        "element normal active" = {
          text-color = mkLiteral "@fg1";
        };

        "element selected normal" = {
          text-color = mkLiteral "@bg0"; #1
          background-color = mkLiteral "@bg3";
        };
        "element selected active" = {
          text-color = mkLiteral "@bg0"; #1
          background-color = mkLiteral "@bg3";
        };

        "element selected urgent" = {
          background-color = mkLiteral "@fg1";
        };

        "element-icon" = {
          size = mkLiteral "0.8em";
        };

        "element-text" = {
          text-color = mkLiteral "inherit";
        };

        "scrollbar" = {
          handle-width = mkLiteral "4px";
          handle-color = mkLiteral "@fg1";
          padding = mkLiteral "0 4px";
        };
      };
    };
  };
}

